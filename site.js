...
function showAll() {
    map.eachLayer(  
    function(l) {
       if ( typeof(l._tilejson) == 'undefined') {map.removeLayer(l); }
     })
...
for ( var i = 0; i < geoJson.length; i++ ){
    ...
    var marker = new L.marker(new L.LatLng(coords[1], coords[0]), {
        icon:  L.mapbox.marker.icon({'marker-symbol': feature.properties['marker-symbol'], 'marker-color': feature.properties['marker-color'], "marker-size": 'medium'}),
...
map.addLayer(markers)
    ...
    all.bind('click',function(){
        $('#map-ui .active').each(function(){$(this).removeClass('active')});
